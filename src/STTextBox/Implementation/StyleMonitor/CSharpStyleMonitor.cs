...
namespace TSkin.ST
{
    public class CSharpStyleMonitor : TextStyleMonitor
    {
        ...
        private int CheckDeclare(List<Symbol> lst, int nIndex)
        {
            ...
            if (lst[nTemp].Value == "operator")
            { // Overloaded operator functions are special. So deal with it independently.
                lst[nTemp].Type = SymbolType.KeyWord;
        ...
        private int CheckFunction(List<Symbol> lst, int nIndex)
        {
            ...
            if (nTemp < 0) return 0;
            // now -> lst[nTemp] maybe is DATA_TYPE;
            if (this.CheckIsBoundary(lst[nTemp].Value[0]))
            {   // It's a function call like ".XXX(" or ";XXX("
                return 0;
            }
            // other cases, it's a function declaration
            if (lst[nTemp].Type == SymbolType.None)
            {          // if none ,so it's not generic
        ...
        private int CheckAttribute(List<Symbol> lst, int nIndex)
        {
            ...
            if (lst[nIndex + 1].Value == "]")
            { // maybe just a array like -> new int[][]();
                return 2;
            }
            if (lst[nIndex + 2].Value == "]")
            { // maybe just a array like -> new int[2][2]();
            ...
            for (int i = nIndex + 2; i < lst.Count; i++)
            {
                ...
                {
                    if (lst[i - 2].Value == ".")
                    {                  // like this [attr(XXX.xx, xx.XXX.xx)]
        ...
        private void ProcessEqual(List<Symbol> lst, int nIndex)
        {
            ...
            if (nIndex + 1 < lst.Count && lst[nIndex + 1].Value == "=")
            {   // == return
                return;
            }
            if (--nIndex < 0) return;
