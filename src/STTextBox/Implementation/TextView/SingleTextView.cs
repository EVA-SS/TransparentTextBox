...
namespace TSkin.ST
{
    class SingleTextView : TextView
    {
        ...
        public override void OnTextStartChange(ISTTextBoxRender render, TextManagerTextEventArgs e)
        {
            ...
            if (this.Core.TextBox.Password)
                for (int i = 0; i < e.TextHistoryRecord.Count; i++)
                {
                    var r = e.TextHistoryRecord[i];
                    r.RealText = r.NewText = r.NewText.Replace("\r", "").Replace("\n", "");
                    r.NewText = GetPass(r.NewText.Length);
        ...
        public override FindInfo FindFromPoint(Point pt)
        {
            ...
            int nIndexLine = c.Scroll.YValue + ((pt.Y - base.TextRectangle.Y) / c.LineHeight);
            if (nIndexLine >= c.TextManager.LineCount)
            {
