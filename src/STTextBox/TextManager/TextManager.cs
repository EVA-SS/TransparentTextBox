...
namespace TSkin.ST
{
    public class TextManager : IEnumerable<TextLine>
    {
        ...
        public StringBuilder GetStringBuilder(int nIndex, int nLen)
        {
            ...
            {
                sb.Append(stLineStart.RealString.Substring(nIndex - stLineStart.IndexOfFirstChar, nLen));
                return sb;
            }
            int nLenAdded = 0;
            sb.Append(stLineStart.RealString.Substring(nIndex - stLineStart.IndexOfFirstChar));
            nLenAdded += sb.Length;
            for (int i = nIndexStart + 1; i < nIndexEnd; i++)
            {
                sb.Append(this[i].RealString);
                nLenAdded += this[i].RealString.Length;
            }
            sb.Append(stLineEnd.RealString.Substring(0, nLen - nLenAdded));
            return sb;
        }
        ...
        private TextHistoryRecord SetText(int nIndex, int nLen, string strText, bool isHistory)
        {
            ...
            List<TextLine> lst = this.GetTexLines(historyRecord.NewText, historyRecord.RealText);
            ...
            string strRealLeft = line_start.RealString.Substring(0, nIndex - line_start.IndexOfFirstChar);
            string strRealRight = line_end.RealString.Substring(nIndex + nLen - line_end.IndexOfFirstChar);

            lst[0].RawString = strLeft + lst[0].RawString;
            lst[0].RealString = strRealLeft + lst[0].RealString;
            lst[0].IndexOfFirstChar = line_start.IndexOfFirstChar;
            lst[lst.Count - 1].RawString += strRight;
            lst[lst.Count - 1].RealString += strRealRight;
            int nIncrement = historyRecord.NewText.Length - nLen;
            m_nTextLength += nIncrement;

            line_start.RawString = lst[0].RawString;
            line_start.RealString = lst[0].RealString;
        ...
        private List<TextLine> GetTexLines(string strText, string readText)
        {
            ...
            for (int i = 0; i < strText.Length; i++)
            {
                ...
                if (c == '\n' || c == '\r')
                {
                    ...
                    line.RealString = readText.Substring(nIndexStart, nLen);
                    line = new TextLine();
                    lst.Add(line);
                    nIndexStart = i + 1;
                    nLen = 0;
